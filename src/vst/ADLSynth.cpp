...
ADLSynth::ADLSynth (audioMasterCallback audioMaster)
...
void ADLSynth::setParameter (VstInt32 index, float value)
{
	lock.acquire();
	switch (index)
	{
...
float ADLSynth::getParameter (VstInt32 index)
{
	float value = 0;
	switch (index)
	{
...
void ADLSynth::getParameterDisplay (VstInt32 index, char* text)
{
	if (!text)
	{
		return;
...
void ADLSynth::getParameterLabel (VstInt32 index, char* label)
{
	if (!label)
	{
		return;
...
void ADLSynth::getParameterName (VstInt32 index, char* name)
{
	if (!name)
	{
		return;
...
VstInt32 ADLSynth::setChunk (void* data, VstInt32 byteSize, bool isPreset)
{
	if (!data)
	{
		return 0;
	}
	ADLSynthChunk* chunkData = (ADLSynthChunk*)data;
	VstInt32 i;
	if (chunkData->Size != sizeof(ADLSynthChunk))
	{
		return 0;
	}
...
VstInt32 ADLSynth::getChunk (void** data, bool isPreset)
{
	if (!data)
	{
		return 0;
	...
	VstInt32 i;
	chunk.Size = sizeof(ADLSynthChunk);
	...
	*data = &chunk;
	return sizeof(ADLSynthChunk);
}

void ADLSynth::setProgram (VstInt32 program)
{
...
VstInt32 ADLSynth::getProgram ()
{
	return curProgram%kNumPrograms;
}

void ADLSynth::setProgramName (char* name)
{
	if (!name)
	{
		return;
...
void ADLSynth::getProgramName (char* name)
{
	if (!name)
	{
		return;
...
bool ADLSynth::string2parameter (VstInt32 index, char* text)
{
	if (!text)
	{
		return false;
...
bool ADLSynth::getParameterProperties (VstInt32 index, VstParameterProperties* p)
{
	if (!p)
	{
		return false;
...
bool ADLSynth::getProgramNameIndexed (VstInt32 category, VstInt32 index, char* text)
{
	if (index == 0 && text)
	{
...
bool ADLSynth::getInputProperties (VstInt32 index, VstPinProperties* properties)
{
	if (!properties)
	{
		return false;
...
bool ADLSynth::getOutputProperties (VstInt32 index, VstPinProperties* properties)
{
	if (!properties)
	{
		return false;
...
bool ADLSynth::setBypass (bool onOff)
{
	lock.acquire();
	bypassed = onOff;
	if (bypassed)
...
bool ADLSynth::getEffectName (char* name)
{
	if (!name)
	{
		return false;
	}
	vst_strncpy (name, "ADLSynth", kVstMaxEffectNameLen-1);
	return true;
}

bool ADLSynth::getVendorString (char* text)
{
	if (!text)
	{
		return false;
...
bool ADLSynth::getProductString (char* text)
{
	if (!text)
	{
		return false;
	}
	vst_strncpy (text, "ADLSynth VSTi", kVstMaxProductStrLen-1);
#ifdef DEMO
...
VstInt32 ADLSynth::getVendorVersion ()
{ 
	return 1000; 
}

VstPlugCategory ADLSynth::getPlugCategory ()
{
	return kPlugCategSynth;
}

VstIntPtr ADLSynth::vendorSpecific (VstInt32 lArg, VstIntPtr lArg2, void* ptrArg, float floatArg)
{
	switch (lArg)
	{
	...
	case effGetEffectName:
		...
		{
			if (renamePlug ((char**)ptrArg, "ADLSynth"))
			{
				return 0xf00d;
			}
		}
...
VstInt32 ADLSynth::canDo (char* text)
{
	if (!text)
	return 0;
...
#if VST_2_4_EXTENSIONS
VstInt32 ADLSynth::getNumMidiInputChannels ()
{
	return 16;
}

VstInt32 ADLSynth::getNumMidiOutputChannels ()
{
	if (hi.ReceiveEvents)
	{
		return 16;
