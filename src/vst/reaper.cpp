...
bool ADLSynth::getParameterDisplayValue (VstInt32 index, char* text, float value)
{
	if (!text)
	{
		return false;
...
bool ADLSynth::string2parameterReplace (VstInt32 index, char* text)
{
	if (!text)
	{
		return false;
...
bool ADLSynth::isEnumParameter (VstInt32 index)
{
	switch (index)
	{
	case kVolumeDisplay:
...
bool ADLSynth::automateParameter (VstInt32 index, float value, VstInt32 timestamp)
{
...
bool ADLSynth::parameterRange (VstInt32 index, double* range)
{
	if (!range)
	{
		return false;
...
bool ADLSynth::renamePlug (char** text, const char* newName)
{
	if (!text || !newName)
	{
		return false;
...
void ADLSynth::adjustParameterIndex (VstInt32 index, VstInt32 adjust)
{
