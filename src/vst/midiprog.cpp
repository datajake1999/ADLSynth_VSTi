...
VstInt32 ADLSynth::getMidiProgramName (VstInt32 channel, MidiProgramName* mpn)
{
...
VstInt32 ADLSynth::getCurrentMidiProgram (VstInt32 channel, MidiProgramName* mpn)
{
	if (channel < 0 || channel >= 16 || !mpn)
	return -1;
	VstInt32 prg = synthCurrentProgram(&synth, channel);
...
void ADLSynth::fillProgram (VstInt32 channel, VstInt32 prg, MidiProgramName* mpn)
{
...
VstInt32 ADLSynth::getMidiProgramCategory (VstInt32 channel, MidiProgramCategory* cat)
{
...
bool ADLSynth::hasMidiProgramsChanged (VstInt32 channel)
{
	if (channel < 0 || channel >= 16)
	return false;
	if (synthCurrentProgram(&synth, channel) != channelPrograms[channel])
	{
		return true;
	}
	return false;
...
bool ADLSynth::getMidiKeyName (VstInt32 channel, MidiKeyName* key)
